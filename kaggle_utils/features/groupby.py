...
class DiffGroupbyTransformer(BaseGroupByTransformer):
    ...
    def _aggregate(self, dataframe):
        for param_dict in self.param_dict:
            ...
            g = dataframe[all_features].groupby(key)[var].agg(agg).reset_index()
            ...
            if self.additional_stats:
                additional_new_features = self._get_feature_names(
                    key, new_features, agg, prefix=False
                )
                features = (
                    features.groupby(key)[new_features]
                    .agg(self.additional_stats)
                    .reset_index()
                )
    ...
    def _get_feature_names(self, key, var, agg, prefix=True):
        ...
        if prefix:
            return [
                "_".join(["diff", a, v, "groupby"] + key) for v in var for a in _agg
            ]
        else:
            return ["_".join([a, v, "groupby"] + key) for v in var for a in _agg]
...
class RatioGroupbyTransformer(BaseGroupByTransformer):
    ...
    def _get_feature_names(self, key, var, agg, prefix=True):
        ...
        if prefix:
            return [
                "_".join(["ratio", a, v, "groupby"] + key) for v in var for a in _agg
            ]
        else:
            return ["_".join([a, v, "groupby"] + key) for v in var for a in _agg]
...
class LagGroupbyTransformer(BaseGroupByTransformer):
    ...
    def _aggregate(self, dataframe):
        ...
        for param_dict in self.param_dict:
            ...
            new_features = self._get_feature_names(key, var, agg)
            features = (
                dataframe[all_features].groupby(key)[var].shift(-self.shift)
                - dataframe[var]
            )
    ...
    def _get_feature_names(self, key, var, agg):
        return [
            "_".join([a, str(self.shift), v, "groupby"] + key) for v in var for a in agg
        ]
        ...
        for param_dict, features in zip(self.param_dict, self.features):
            ...
            if merge:
                dataframe = dataframe.merge(
                    features, how="left", left_on=on, right_on=key
                )
            else:
                new_features = self._get_feature_names(key, var, agg)
                dataframe = pd.concat([dataframe, features[new_features]], axis=1)
...
class EWMGroupbyTransformer(BaseGroupByTransformer):
    ...
    def _aggregate(self, dataframe):
        ...
        for param_dict in self.param_dict:
            ...
            features = (
                ...
                .groupby(key)[var]
                .transform(self.calc_shifted_ewm)
            )
            features = features.fillna(self.fill_na)
            features.columns = new_features
            self.features.append(features)
    ...
    def _get_feature_names(self, key, var, agg):
        return [
            "_".join([a, str(self.alpha), v, "groupby"] + key) for v in var for a in agg
        ]
...
class BayesianMeanGroupbyTransformer(BaseGroupByTransformer):
    ...
    def _aggregate(self, dataframe):
        for param_dict in self.param_dict:
            ...
            features[new_features[0]] = (
                (features[var[0]] / features["mean"]) * features["count"] + self.l
            ) / (features["count"] + self.l)
            features = change_dtype(features, columns=new_features)
    ...
    def _get_feature_names(self, key, var, agg):
        return [
            "_".join([a, str(self.l), v, "groupby"] + key) for v in var for a in agg
        ]
...
class TargetEncodingTransformer(BaseGroupByTransformer):
    ...
    def __init__(
        self, target, n_splits, cvfold, len_train, min_count=1, param_dict=None
    ):
        """
            Params
            ------
        ...
        if isinstance(cvfold, pd.DataFrame):
            for fold_id in range(self.n_splits):
                train_index = np.array(
                    self.cvfold["train_id" + str(fold_id)] == fold_id
                ).flatten()
    ...
    def _aggregate(self, dataframe):
        ...
        for param_dict in self.param_dict:
            ...
            feature = pd.DataFrame(
                np.empty([len(dataframe), len(self.var) * len(agg)]),
            ...
            for train_index, valid_index in self._split_kfold(self.cvfold):
                ...
                feature.iloc[: self.len_train, :].loc[valid_index] = val[
                    new_features
                ].values
...
class BayesianTargetEncodingTransformer(TargetEncodingTransformer):
    ...
    def _encode(self, dataframe, merge_dataframe, key, var, agg, new_features):
        ...
        g = change_dtype(g, columns=new_features)
        merge_dataframe = merge_dataframe.merge(
            g[key + new_features], on=key, how="left"
        )
        return merge_dataframe
...
class Seq2DecTargetEncodingTransformer(TargetEncodingTransformer):
    ...
    def _encode(self, dataframe, merge_dataframe, key, var, agg, new_features):
        ...
        g.columns = key + new_features
        g[new_features[0]] = (
            g[new_features[0]].apply(lambda x: x[:1] + "." + x[1:]).astype(float)
        )
        merge_dataframe = merge_dataframe.merge(g, on=key, how="left")
        return merge_dataframe
    ...
    def _aggregate(self, dataframe):
        ...
        for param_dict in self.param_dict:
            ...
            new_features = self._get_feature_names(key, var, agg)
            feature = pd.DataFrame(
                np.empty([len(dataframe), len(self.var)]), columns=new_features
            )
