...
class OOFCategoryEncoder(BaseFeatureTransformer):
    def __init__(
        ...
        if encoder is None:
            self.encoder = CatBoostEncoder(cols=categorical_features, return_df=False,)
        else:
    ...
    def fit_transform(self, X, y):
        ...
        for fold_id in range(self.n_splits):
            ...
            self.cbe_.append(
                clone(self.encoder).fit(
                    X.loc[train_index, self.categorical_features], y[train_index]
                )
            )
